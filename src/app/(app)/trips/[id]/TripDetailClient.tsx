...
export default function TripDetailClient({
  ...
  // Group items by dayNumber (null = ungrouped)
    ...
    <div className="pb-20 pt-2">
      ...
      <div className="px-5 pt-4 pb-1">
        <button
          ...
          className="text-[13px] text-gray-400 mb-3 flex items-center gap-1 active:opacity-60"
        >
          ← Back to trips
        </button>
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          {isOwner && (
            <button
              ...
              disabled={toggling}
              className={`text-[11px] font-medium px-3 py-1.5 rounded-full transition-all duration-200 flex-shrink-0 ${
        ...
        {!isOwner && (
          ...
          >
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-sky-100 to-indigo-100 overflow-hidden flex-shrink-0">
              {data.user.avatarUrl ? (
                <img
                ...
                <div className="w-full h-full flex items-center justify-center text-[10px] font-semibold text-sky-600">
                  {data.user.name[0]}
                </div>
              )}
            </div>
            <span className="text-[13px] text-gray-500">
              by <span className="font-medium text-gray-700">{data.user.name}</span>
            </span>
          </Link>
        )}
      </div>
...
function ItemCard({ item, index }: { item: ItineraryItem; index: number }) {
  ...
  return (
    <div
      className="flex gap-3.5 bg-white rounded-2xl border border-gray-100/80 p-3.5 animate-fade-in-up"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {item.notes && (
          <p className="text-[12px] text-gray-400 mt-1.5 italic">{item.notes}</p>
        )}
      </div>
